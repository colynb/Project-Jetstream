...
<ul class="tabs">
	...
	foreach ( $event_types as $id => $label ):
		$class = ($id == $named['event_type']) ? 'active' : '';
		$events_url = $this->route->url('event-calendar', array('event_type' => $id));
		?>
		<li><a class="<?= $class ?>" href="<?= $events_url ?>"><?= $label ?></a></li>
	<?php endforeach; ?>
</ul>
